...
| 模式 | 时间 | 适合人群 | 说明 |
|------|------|----------|------|
| **快速配置** | ~3 分钟 | 新手用户、快速体验 | 自动完成环境搭建，只需配置大模型端点 |
...
你只需要完成 **大模型端点配置**（必填）和 **IM 通道配置**（可选）。
...
你至少需要添加 **1 个大模型端点** 才能开始使用。点击「+ 添加端点」按钮打开端点配置对话框：
...
> 如果尚未添加任何大模型端点，按钮会处于禁用状态，并提示「请先添加至少 1 个大模型端点」。
...
- `.env` — 环境变量配置
- `data/llm_endpoints.json` —大模型端点列表
