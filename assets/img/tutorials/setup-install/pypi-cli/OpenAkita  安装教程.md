...
- LLM API Key：OpenAkita 本身是一个框架，需要连接大模型才能思考，如果没有API Key，可以去对应官网注册（如阿里云、Deepseek、Anthropic等）
...
2. 首次启动会弹出配置向导，按照提示配置你的 **LLM API Key**（如 DeepSeek, Anthropic, OpenAI 等）。
...
3. 配置LLM端点：你至少需要添加 **1 个 LLM 端点** 才能开始使用。
...
4. 配置IM通道：暂时可不填，直接跳过
   ...
   **注意**：可在配置向导页面选择暂不配置LLM和IM通道，进入OpenAkita后再进行配置
...
LLM智能审查，将由大模型逐条审查所有记忆，删除垃圾、合并重复，此操作可能需要较长时间，请耐心等待。
